 ...
 *
 *  Perf injects:
 *   - Link rebuild every 3 frames (with “fast pointer” immediate rebuild)
 *   - Cached debug DOM refs + 10fps debug updates
 *   - Sprite-based stars (WebP) with opacity/twinkle preserved
 *   - Faster edge fade (keeps radius in the edge distance)
 *==============================================================*/

//alert("Debug HAT");

/*========================================*
//#region 0) PERF GLOBALS (SPRITES + DEBUG + LINK THROTTLE)
 *========================================*/

var S = window.STARFIELD;

/*---------- 0A) Star sprites (WebP) ----------*/
/**
 * Uses alpha-enabled WebP sprites:
 *   /Resources/Star1.webp
 *   /Resources/Star2.webp
 *   /Resources/Star3.webp
 *
 * Opacity/twinkle still works because we apply STAR.opacity via globalAlpha
 * and keep STAR.whiteValue logic intact (used for physics radius + wrap/bounce).
 */
const STAR_SPRITES = {
  ready: false,
  imgs: []
...
function loadStarSprites() {
  ...
  for (const SRC of FILES) {
    ...
    IMG.onerror = () => {
      LOADED++;
      if (LOADED === FILES.length) STAR_SPRITES.ready = true; // allow running with partial loads
    };
    IMG.src = SRC;
...
document.addEventListener("DOMContentLoaded", loadStarSprites);

/*---------- 0B) Debug DOM refs (cached) + 10fps throttle ----------*/
const DBG = {
  misc: null,
  circle: null,
  speed: null,
...
document.addEventListener("DOMContentLoaded", initDebugRefs);

/*---------- 0C) Link rebuild throttle (every 3 frames) + fast pointer override ----------*/
...
/*---------- 0D) Faster edge fade (keeps radius in edge check) ----------*/
/**
 * 0 at/over edge, 1 safely away. Includes STAR_RADIUS to avoid visible teleport pop.
 * Simple clamp (+ optional squaring for softness) instead of smoothstep.
 */
function getEdgeFadeFastWithRadius(STAR) {
  ...
  // tiny easing without smoothstep cost
  t = t * t;

  return t;
}

/* #endregion 0) PERF GLOBALS */
...
S.updateStarPhysics = function updateStarPhysics() {
  ...
  if (!S.isCanvasReady || !S.starList.length) return;
  ...
  for (const STAR of S.starList) {
    ...
    // Step 19: opacity cycle (twinkle logic preserved)
  ...
  // Step 24: debug readouts (cached refs, 10fps)
  const NOW = S.getNowMs();
  if (NOW - DBG.lastMs >= 100) {
    DBG.lastMs = NOW;

    if (DBG.misc) DBG.misc.textContent = S.starList[0]?.momentumX ?? 0;
...
S.renderStarsAndLinks = function renderStarsAndLinks() {
  if (!S.isCanvasReady || !S.drawingContext) return;
  ...
  // Step 5: draw links (rebuild every 3 frames, but rebuild immediately if pointer is fast)
  CONTEXT.lineWidth = 1;
  ...
  if (STAR_COUNT) {
    ...
    // “Fast pointer” override: force rebuild when user is really moving
    ...
    if (SHOULD_REBUILD_LINKS) {
      ...
      // Step 5a: compute edge fade factor (fast version, keeps radius)
      ...
      // Step 5b: pairwise linking (ONLY on rebuild frames)
    ...
    // Step 5c: stroke each bucket once (EVERY frame, uses cached paths)
    for (let BUCKET_INDEX = 0; BUCKET_INDEX < LINK_BUCKET_COUNT; BUCKET_INDEX++) {
  ...
  // Step 6: draw star bodies (sprite-based, preserves twinkle via STAR.opacity)
  for (const STAR of S.starList) {
    // Fallback if sprites aren’t ready (keeps behavior intact)
    ...
    // Keep your “effective radius” logic (whiteValue still matters for size)
    const R = (STAR.whiteValue * 2 + STAR.size) || 1;
    const SIZE = Math.max(2, R * 2.4);

    // Apply opacity fade/twinkle through alpha
...
S.updatePointerSpeed = function updatePointerSpeed(CURRENT_X, CURRENT_Y) {
  ...
  // Step 5.5: fast pointer -> request link rebuild soon
  if (S.pointerSpeedUnits > 10) LINKS_DIRTY = true;

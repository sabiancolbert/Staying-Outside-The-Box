...
function moveStars() {
  ...
  for (const STAR of STARS) {
    ...
    // Decay momentum
    STAR.momentumX = Math.sign(STAR.momentumX) * Math.max(0, Math.abs(STAR.momentumX) - 0.01);
    STAR.momentumY = Math.sign(STAR.momentumY) * Math.max(0, Math.abs(STAR.momentumY) - 0.01);
